function [fig1,fig2,fig3,fig4,fig5,fig6] = ComparisonS1Function(results,nCases)
...
for iCase = 1:nCases  
    ...
    % Taper ratio
    taper(iCase) = results(iCase).taper;
end

% Optional: sort by taper ratio
[taper, idx] = sort(taper);
...
plot(taper, Pi, '-o','LineWidth', 2,'MarkerSize', 7);
set(gca, 'FontSize', 12);
...
%% COMMON FORMATTING
xlabel('Taper Ratio $[\lambda]$', 'Interpreter', 'latex', 'FontSize', 16.5);

title('\textbf{Power Variation with Taper Ratio}', 'Interpreter', 'latex', ...
    'FontSize', 18);
...
plot(taper, FM, '-o','LineWidth', 2,'MarkerSize', 7);

set(gca, 'FontSize', 12);
title('$\textbf{Figure of Merit Variation with Taper Ratio}$', 'Interpreter', 'latex', 'FontSize', 18)
xlabel('Taper Ratio $[\lambda]$', 'Interpreter', 'latex', 'FontSize', 16.5)
...
hold on;
plot(taper,w_tip,'-o','LineWidth', 2,'MarkerSize', 7);
xlim([min(taper) max(taper)])
grid on;
set(gca, 'FontSize', 12);
...
hold on;
plot(taper,twist,'LineWidth', 2,'MarkerSize', 7);
plot(taper,rot_y,'LineWidth', 2,'MarkerSize', 7);
xlim([min(taper) max(taper)])
grid on;
set(gca, 'FontSize', 12);
...
hold on;
plot(taper,Mx,'LineWidth', 2,'MarkerSize', 7);
plot(taper,My,'LineWidth', 2,'MarkerSize', 7);
xlim([min(taper) max(taper)])
grid on;
set(gca, 'FontSize', 12);
...
hold on;
plot(taper,Nx,'LineWidth', 2,'MarkerSize', 7);
xlim([min(taper) max(taper)])
grid on;
set(gca, 'FontSize', 12);
